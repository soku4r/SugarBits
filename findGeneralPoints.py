    ...
    for y in range(len(h_lons)):
        for p in range(len(h_plev)):
            ...
            while j < len(h_time):
                ...
                if(h_average_day < 45 and h_average_night < 45) or (t_average_day > 35 or t_average_day < 6) or (t_average_night < 3 or t_average_night > 35) or(abs(t_average_day - t_average_night) <3) :
                    bad_day_counter = bad_day_counter + 1
                else:
                    bad_day_counter = 0
                if(bad_day_counter > 15 or t_average_day < -5 or t_average_night < -8):
                    possible = False
                    #print(j, "stop" )
                    break
